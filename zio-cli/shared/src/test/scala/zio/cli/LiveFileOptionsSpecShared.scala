...
/**
 * Cross-platform tests for `FileOptions.Live`. Uses `TestSystem` to override `user.dir` / `user.home` so we never
 * depend on the actual machine the test is running on.
 */
